...
open class BlePeripheral: NSObject {
    ...
    func discover(characteristicUuids: [CBUUID]?, serviceUuid: CBUUID, completion: ((Error?) -> Void)?) {
        // Discover service
        print("discover characteristicUuids");
    ...
    private func handlerIdentifier(from characteristic: CBCharacteristic) -> String {
        guard let service = characteristic.service else { print("Error: handleIdentifier with nil characteritic service"); return "" }
    ...
    private func handlerIdentifier(from descriptor: CBDescriptor) -> String {
        guard let characteristic = descriptor.characteristic, let service = characteristic.service else { print("Error: handleIdentifier with nil descriptor service"); return "" }
...
extension BlePeripheral: CBPeripheralDelegate {
    ...
    public func peripheral(_ peripheral: CBPeripheral, didReadRSSI RSSI: NSNumber, error: Error?) {
        guard error == nil else { print("didReadRSSI error: \(error!.localizedDescription)"); return }
