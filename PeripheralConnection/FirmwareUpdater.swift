...
//    }
    ...
    private func servicesDiscovered(peripheral: BlePeripheral, delegate: FirmwareUpdaterDelegate, shouldRecommendBetaReleases: Bool, versionToIgnore: String?) {
        guard let _ = peripheral.discoveredService(uuid: FirmwareUpdater.kDfuServiceUUID), let disService = peripheral.discoveredService(uuid: FirmwareUpdater.kDisServiceUUID) else {
            print("Updates: Peripheral has no DFU or DIS service available")
        ...
        print("Discover DIS Characteristics")
        ...
        peripheral.discover(characteristicUuids: nil, service: disService) { [weak self] error in
            ...
            guard error == nil else {
                print("Error discovering DIS characteristics")
                return
            }

            print("Read DIS characteristics")
            ...
            dispatchGroup.notify(queue: .global(), execute: { [weak strongSelf] in
                print("Device Info Data received")
    ...
    private func readCharacteristic(uuid: CBUUID, peripheral: BlePeripheral, service: CBService, dispatchGroup: DispatchGroup, completion: @escaping ((String?) -> Void)) {
        ...
        if let characteristic = peripheral.discoveredCharacteristic(uuid: uuid, service: service) {
            ...
            print("Warning: updates check didn't find characteristic: \(uuid.uuidString)")
            dispatchGroup.leave()
        }
    }
                            ...
                            if !modelReleases.isEmpty {
                                ...
                                if let latestRelease = latestRelease, let minBootloaderVersion = latestRelease.minBootloaderVersion, let bootloaderVersion = deviceDfuInfo.bootloaderVersion, bootloaderVersion.compare(minBootloaderVersion, options: [.numeric]) != .orderedAscending {
                                    ...
                                    if versionToIgnore == nil || latestRelease.version.compare(versionToIgnore!, options: [.numeric]) != .orderedSame {
                                        ...
                                        #if DEBUG
                                        if isNewerVersion {
                                            print("Updates: New version found. Ask the user to install: \(latestRelease.version)")
                                        } else {
                                            print("Updates: Device has already latest version: \(deviceDfuInfo.softwareRevision ?? "<unknown>")")
                                        }
                                        #endif
                                    } else {
                                        print("Updates: User ignored version: \(versionToIgnore ?? "<unknown>"). Skipping...")
                                    }
                                } else {
                                    print("Updates: Bootloader version \(deviceDfuInfo.bootloaderVersion ?? "<unknown>") below minimum needed: \(latestRelease?.minBootloaderVersion ?? "<unknown>")")
                                }
                            } else {
                                print("Updates: No firmware releases found for model: \(deviceDfuInfo.modelNumber ?? "<unknown>")")
                            }
                        } else {
                            print("Updates: No releases found for model: \(deviceDfuInfo.modelNumber ?? "<unknown>")")
                        }
                    } else {
                        print("Updates: No updates for unknown manufacturer: \(deviceDfuInfo.manufacturer ?? "<unknown>")")
                    }
                } else {
                    print("Updates: The legacy bootloader on this device is not compatible with this application")
                }
            } else {
                print("Updates: firmwareRevision not defined")
            }
        } else {
            print("Updates: releases array is empty")
        }
